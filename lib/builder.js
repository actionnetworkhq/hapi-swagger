...
builder.getSwaggerJSONFromServer = async (server, settings, { host, schemes, queryTags }) => {
  // remove items that cannot be changed by user
  delete settings.swagger;
  delete settings.templates;

  // collect root information
  builder.default.host = host;
  builder.default.schemes = [schemes];
  ...
  let filteredRoutes = server.table();

  // filter routes displayed based on tags passed in query string
  if (queryTags && queryTags.length) {
    filteredRoutes = Filter.byTags(queryTags, filteredRoutes);
  }


  if (typeof settings.routeTag === 'function') {
    filteredRoutes = Filter.byFunction(settings.routeTag, filteredRoutes);
  } else {
    filteredRoutes = Filter.byTags([settings.routeTag], filteredRoutes);
  }

  Sort.paths(settings.sortPaths, filteredRoutes);

  // append group property - by path
  Group.appendGroupByPath(settings.pathPrefixSize, settings.basePath, filteredRoutes, settings.pathReplacements);

  const paths = new Paths(settings);
  const pathData = paths.build(filteredRoutes);
...
/**
 * gets the Swagger JSON from Hapi request
 *
 * @param  {Object} settings
 * @param  {Request} request
 */
builder.getSwaggerJSONFromRequest = (settings, request) => {
  const host = internals.getHostFromRequest(request);
  const schemes = internals.getSchema(request);

  let queryTags;

  if(request.query.tags) {
    queryTags = request.query.tags.split(',');
  }

  return builder.getSwaggerJSONFromServer(request.server, settings, { host, schemes, queryTags });
};
 ...
 * @return {string}
 */
internals.getHostFromRequest = function(request) {
