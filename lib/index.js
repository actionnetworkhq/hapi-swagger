...
const schema = Joi.object({
  ...
  jsonFilePath: Joi.string().allow(null),
...
const getFullSettings = (options, logger) => {
...
exports.plugin = {
  ...
  register: (server, options) => {
    ...
    const settings = getFullSettings(options, server.log.bind(server));

    settings.log(['info'], 'Started');

    if(!server.methods.getSwaggerJSON) {
      let getJSON;
      let options;

      if(settings.jsonFilePath) { // add server method for static file
        getJSON = async (settings, request) => {
          try {
            await FS.access(settings.jsonFilePath);

            // file exists

            const [stat, contents] = await Promise.all([
              FS.stat(settings.jsonFilePath),
              FS.readFile(settings.jsonFilePath),
            ]);

            return {
              cached: { stored: stat.ctime },
              value: JSON.parse(contents),
            };
          } catch(err) { // haven't saved file yet
            settings.log(['info'], 'Generating static Swagger JSON');

            const value = await Builder.getSwaggerJSONFromRequest(settings, request);

            await FS.writeFile(settings.jsonFilePath, JSON.stringify(value));

            return {
              cached: { stored: new Date() },
              value,
            };
          }
        };
      } else if(settings.cache) { // add server method for caching
        // set default
        settings.cache.segment = Pack.name;
        settings.cache.getDecoratedValue = true;
        if (!settings.cache.generateTimeout) {
          settings.cache.generateTimeout = 30 * 1000;
        }

        getJSON = Builder.getSwaggerJSONFromRequest;

        // If you need access to the cache result envelope information { value, ttl, report },
        // use the catbox getDecoratedValue option.
        options = {
          cache: settings.cache,
          generateKey: (settings, request) => Pack.name + request.path
        };
      }

      if(getJSON) {
        server.method('getSwaggerJSON', getJSON, options);
      }
    }
    ...
    server.route([
      {
        ...
        options: Hoek.applyToDefaults({
          ...
          handler: async (request, h) => {
            if (settings.cache || settings.jsonFilePath) {
              ...
              const json = await Builder.getSwaggerJSONFromRequest(settings, request);
              return json;
    ...
    server.expose('getJSON', (exposeOptions, request, callback) => {
      ...
      return Builder.getSwaggerJSONFromRequest(exposeSettings, request, callback);
    });
    /* $lab:coverage:on$ */
  }
};
...
exports.builder = {
  getSwaggerJSONFromServer: (server, pluginSettings, options, logger = () => {}) => {
    const settings = getFullSettings(pluginSettings, logger);

    return Builder.getSwaggerJSONFromServer(server, settings, options);
  }
};
